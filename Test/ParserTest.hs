...
emptyTest = TestCase (assertEmpty result)
    where
        result   = parseLdd tsrc ""

staticTest :: Test
staticTest = TestCase (assertEmpty result)
    where
        result   = parseLdd tsrc "\tstatically linked"

normalTest :: Test
normalTest = TestCase (assertSoInfo expected result)
    where
        expected = ["/path/liba.so.7"]
        result   = parseLdd tsrc ("\tliba.so.7 => /path/liba.so.7 (0xaa)")

pathTest :: Test
pathTest = TestCase (assertEmpty result)
    where
        result   = parseLdd tsrc "\t/path/liba.so.9 (0xaa)"

nameTest :: Test
nameTest = TestCase (assertEmpty result)
    where
        result   = parseLdd tsrc "\tliba.so.8 => (0xaa)"
